...
def multiple_bar_chart(xvalues: list, yvalues: dict, float_vals: bool = True, ax: plt.Axes = None, title: str = '', xlabel: str = '', ylabel: str = '',
    ...
    for metric in yvalues:
        ...
        for k in range(len(values)):
            if float_vals:
                ax.text(pos_group[k], values[k] + 0.01, f'{values[k]:.4f}', ha='center', fontproperties=FONT_TEXT)
            else:    
                ax.text(pos_group[k], values[k] + 0.01, values[k], ha='center', fontproperties=FONT_TEXT)
        pos_group = pos_group + width
        i += 1
    ax.legend(legend, fontsize='x-small', title_fontsize='small', loc='upper center', bbox_to_anchor=(0.5, -0.05),
          fancybox=True, shadow=True, ncol=5)


def plot_evaluation_results(labels: np.ndarray, cnf_mtx_trn, cnf_mtx_tst, graphs = True):
    HEIGHT: int = 4
    ...
    evaluation = {
        ...
        'F1': [tp_trn / (tp_trn + 0.5 * (fp_trn + fn_trn)), tp_tst / (tp_tst + 0.5 * (fp_tst + fn_tst))]}

    if graphs:
        fig, axs = plt.subplots(1, 2, figsize=(2 * HEIGHT, HEIGHT), gridspec_kw={'width_ratios': [2, 1]})
        multiple_bar_chart(['Train', 'Test'], evaluation, ax=axs[0], title="Model's performance over Train and Test sets", percentage=True)
        mean_conf_matrix_tst = np.around(cnf_mtx_tst, 0).astype(np.int64)
        plot_confusion_matrix(mean_conf_matrix_tst, labels, ax=axs[1], title='Test')
    return evaluation
